...
- **Délégation pédagogique Staff & Gouvernance Proviseur (septembre 2026)** :
  ...
  - **Inscriptions d'élèves en cours d'année (septembre 2026)** : seuls deux rôles peuvent traiter les dossiers : le **Secrétaire** (`STAFF` avec `MANAGE_ENROLLMENT`) par défaut, et l'**Admin** (`ADMIN`) à condition qu'il ait activé l'option `adminGereInscriptions` sur l'établissement (`PATCH /api/v2/eleve-onboarding/admin-gestion`, tracé dans `ActivitiesLog`). **Suppression de toute étape de validation séparée** : l'action d'inscrire finalise immédiatement le dossier (`SOUMIS` → `INSCRIT`) et crée les comptes utilisateurs sans requérir d'approbation supplémentaire par un tiers.
