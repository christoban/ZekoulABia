...
| **subject** (5 UC) | Matières, coefficients, assignation enseignant ↔ matière | `app/subject`, `infra/http/controllers/SubjectController` | `CreerMatiereUseCase`, `AssignerEnseignantMatiereUseCase`, `DefinirCoefficientUseCase` | `MatiereRepository`, `UserRepository` |
| **teachingAssignment** (1 UC) | Affectation enseignant ↔ classe/matière (manuelle + génération glouton) | `app/teachingAssignment`, `infra/http/controllers/TeachingAssignmentController` | `GenererAffectationsUseCase`, `AffecterEnseignantsService` | `RattachementEnseignantRepository`, `TeachingAssignmentGeneratorRepository` |
...
| **timetable** | Emplois du temps, créneaux, publication, rattrapages ; **Scheduling Engine CP-SAT (V2.5)** : propose → apply → what-if ; **génération globale** : `propose-all` (run asynchrone Inngest, step/classe, occupation cumulée) → `apply-all` (transaction atomique multi-EDT) | `app/timetable`, `infra/http/controllers/TimetableController` `TimetableGridConfigController`, `infra/scheduling/`, `infra/inngest/functions/academic.ts` | `ProposerEmploiDuTempsUseCase`, `AppliquerPropositionEmploiDuTempsUseCase`, `AppliquerLotEmploiDuTempsUseCase`, `ProposerEmploisDuTempsGlobalUseCase`, `SimulerEmploiDuTempsUseCase`, `ORToolsWasmAdapter`, `contraintesDouces.ts` | `TimetableRepository`, `TimetableGenerationRunRepository`, `TimetableGenerationTargetProvider`, `SchedulingSolverPort` (or-tools-wasm), Inngest |
...
| **babillard / announcement** (8 UC) | Babillard officiel skeuomorphic (panneau de liège) : communiqués officiels, format A4, épinglage (max 3), ciblage rôles/classes, pièces jointes sécurisées (magic bytes), tracking lecture, programmation, audit | `app/babillard`, `infra/http/controllers/BabillardController` | `CreerPublicationUseCase`, `ModifierPublicationUseCase`, `SupprimerPublicationUseCase`, `ListerPublicationsUseCase`, `MarquerPublicationLueUseCase`, `EpinglerPublicationUseCase`, `CalculerStatistiquesLectureUseCase`, `UploaderPieceJointeUseCase` | `PublicationRepository` (Prisma), `pdf-lib`, `sanitize-html` |
...
| **Persistence** | 28 repositories Prisma | `infra/persistence/prisma/*Repository.ts`, `config/prisma.ts` | Implémentent les ports |
