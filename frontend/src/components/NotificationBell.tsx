  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      ...
      <div
        ...
        style={{ width: 36, height: 36, borderRadius: 9, background: attention ? 'var(--red-light)' : 'var(--bg2)', border: `1.5px solid ${attention ? 'var(--red)' : 'var(--border)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', position: 'relative', transition: 'background 0.2s, border-color 0.2s' }}
      >
        {attention && (
          <span className="zek-bell-ping" style={{ position: 'absolute', inset: -3, borderRadius: 10, background: 'var(--red)', pointerEvents: 'none' }} />
        )}
        <Bell className="zek-bell-icon" size={16} color={attention ? '#b91c1c' : 'var(--text2)'} style={{ transformOrigin: '50% 15%', position: 'relative' }} />
        {unreadCount > 0 && (
          <div className="zek-bell-dot" style={{ position: 'absolute', top: 3, right: 3, minWidth: 15, height: 15, padding: '0 4px', background: 'var(--red)', borderRadius: 7, border: '2px solid var(--surface)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 800, color: 'white' }}>
