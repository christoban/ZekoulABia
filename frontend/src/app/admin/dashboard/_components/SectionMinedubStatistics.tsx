  ...
  return (
    <div className="px-4 py-4 md:px-6 md:py-5" style={{ height: '100%', overflowY: 'auto' }}>
      <div className="mb-[12px] md:mb-[14px]">
        <h2 className="text-[15px] md:text-[17px]" style={{ fontFamily: 'var(--font-spectral),Spectral,serif', fontWeight: 700, color: 'var(--text)' }}>{t('minedubStats.title')}</h2>
        <p className="text-[11px] md:text-[12px]" style={{ color: 'var(--text3)', marginTop: 2 }}>{t('minedubStats.subtitle')}</p>
      </div>

      <div className="text-[11px] md:text-[11.5px] px-[10px] md:px-[12px] py-[7px] md:py-[8px] mb-[12px] md:mb-[14px]" style={{ background: 'rgba(234,179,8,0.1)', border: '1px solid #eab308', borderRadius: 8, color: '#92400e' }}>
        {t('minedubStats.nonOfficialWarning')}
      </div>

      <div className="gap-[6px] md:gap-[8px] mb-[12px] md:mb-[14px]" style={{ display: 'flex' }}>
        <button onClick={() => setTab('supplement')} className="flex-1 md:flex-none rounded-[8px] text-[11.5px] md:text-[12.5px] py-[6px] px-3 md:px-[16px]" style={{ ...(tab === 'supplement' ? btnPri : btnSec), padding: undefined }}>{t('minedubStats.tabSupplement')}</button>
        <button onClick={() => setTab('generer')} className="flex-1 md:flex-none rounded-[8px] text-[11.5px] md:text-[12.5px] py-[6px] px-3 md:px-[16px]" style={{ ...(tab === 'generer' ? btnPri : btnSec), padding: undefined }}>{t('minedubStats.tabGenerer')}</button>
      </div>
      ...
      {tab === 'supplement' && (
        <div>
          ...
          <div className={cardStyleCls} style={cardStyle}>
            <h3 className="text-[13px] md:text-[14px]" style={{ fontWeight: 700, color: 'var(--text)', marginBottom: 4 }}>{t('minedubStats.sectionIdentification')}</h3>
          ...
          <div className={cardStyleCls} style={cardStyle}>
            <h3 className="text-[13px] md:text-[14px]" style={{ fontWeight: 700, color: 'var(--text)', marginBottom: 3 }}>{t('minedubStats.sectionVulnerables')}</h3>
            <p style={{ fontSize: 11.5, color: 'var(--text3)', marginBottom: 10 }}>{t('minedubStats.vulnerablesHint')}</p>
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11.5 }}>
                <thead>
                  <tr>
                    <th style={{ textAlign: 'left', padding: '5px 6px', color: 'var(--text3)', fontWeight: 700 }}>{t('minedubStats.colNiveau')}</th>
                    <th style={{ padding: '5px 6px', color: 'var(--text3)', fontWeight: 700 }}>{t('minedubStats.colRefugiesF')}</th>
                    <th style={{ padding: '5px 6px', color: 'var(--text3)', fontWeight: 700 }}>{t('minedubStats.colRefugiesG')}</th>
                    <th style={{ padding: '5px 6px', color: 'var(--text3)', fontWeight: 700 }}>{t('minedubStats.colDeplacesF')}</th>
                    <th style={{ padding: '5px 6px', color: 'var(--text3)', fontWeight: 700 }}>{t('minedubStats.colDeplacesG')}</th>
                    <th style={{ padding: '5px 6px', color: 'var(--text3)', fontWeight: 700 }}>{t('minedubStats.colHandicapesF')}</th>
                    <th style={{ padding: '5px 6px', color: 'var(--text3)', fontWeight: 700 }}>{t('minedubStats.colHandicapesG')}</th>
                  </tr>
                </thead>
                <tbody>
                  {PRIMARY_LEVELS.map((niveau) => (
                    <tr key={niveau} style={{ borderTop: '1px solid var(--bg)' }}>
                      <td style={{ padding: '3px 6px', fontWeight: 700, color: 'var(--text)' }}>{niveau}</td>
                      {(['refugiesF', 'refugiesG', 'deplacesF', 'deplacesG', 'handicapesF', 'handicapesG'] as const).map((k) => (
                        <td key={k} style={{ padding: '3px 4px' }}>
                          <input style={{ ...smallInputStyle, width: 50 }} type="number" min={0}
          ...
          <div className={cardStyleCls} style={cardStyle}>
            <h3 className="text-[13px] md:text-[14px]" style={{ fontWeight: 700, color: 'var(--text)', marginBottom: 3 }}>{t('minedubStats.sectionInfrastructures')}</h3>
            <p style={{ fontSize: 11.5, color: 'var(--text3)', marginBottom: 10 }}>{t('minedubStats.infrastructuresHint')}</p>
              ...
              {INFRA_ROWS.map((row) => {
                ...
                return (
                  <div key={row.key} style={{ borderTop: '1px solid var(--bg)' }}>
                    ...
                    {isOpen && (
                      <div style={{ padding: '4px 12px 12px', background: 'var(--bg2)', display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                        {INFRA_SUB_KEYS.map((sub) => (
                          <div key={sub.key} style={{ minWidth: 100 }}>
                            <div style={{ fontSize: 10.5, color: 'var(--text3)', marginBottom: 3 }}>{sub.label}</div>
          ...
          <div className={cardStyleCls} style={cardStyle}>
            <h3 className="text-[13px] md:text-[14px]" style={{ fontWeight: 700, color: 'var(--text)', marginBottom: 8 }}>{t('minedubStats.sectionCommodites')}</h3>
      ...
      {tab === 'generer' && (
        <div>
          <div className={cardStyleCls} style={cardStyle}>
            <p className="text-[11.5px] md:text-[12.5px]" style={{ color: 'var(--text2)', marginBottom: 12, lineHeight: 1.5 }}>{t('minedubStats.generateDescription')}</p>
          ...
          {lastReport && (
            <div className={cardStyleCls} style={{ ...cardStyle, border: '1.5px solid var(--green)' }}>
              <p className="text-[12.5px] md:text-[13px]" style={{ fontWeight: 700, color: 'var(--green)', marginBottom: 6 }}>{t('minedubStats.generationSuccess')}</p>
          ...
          <div className={cardStyleCls} style={cardStyle}>
            <h3 className="text-[13px] md:text-[14px]" style={{ fontWeight: 700, color: 'var(--text)', marginBottom: 8 }}>{t('minedubStats.historyTitle')}</h3>
            {reports.length === 0 ? (
              <p style={{ fontSize: 12, color: 'var(--text3)' }}>{t('minedubStats.historyEmpty')}</p>
            ) : (
              <>
              {/* ── Cartes empilées — mobile ── */}
              <div className="md:hidden flex flex-col" style={{ gap: 6 }}>
                {reports.map((r) => (
                  <div key={r.id} className="rounded-[10px] shadow-[0_1px_2px_rgba(20,20,15,0.05)]" style={{ background: 'var(--surface)', padding: '10px 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
                    <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text)' }}>{new Date(r.generatedAt).toLocaleString()}</div>
                    <button onClick={() => downloadReport(r.id)} style={{ padding: '5px 10px', borderRadius: 7, fontSize: 11, fontWeight: 700, background: 'var(--bg2)', color: 'var(--text)', border: 'none', cursor: 'pointer', fontFamily: 'inherit', flexShrink: 0 }}>{t('minedubStats.downloadBtn')}</button>
                  </div>
                ))}
              </div>
              {/* ── Tableau — desktop ── */}
                  ...
                  <tbody>
                    {reports.map((r) => (
                      <tr key={r.id} style={{ borderTop: '1px solid var(--bg)' }}>
                        <td style={{ padding: '6px 10px', color: 'var(--text2)' }}>{new Date(r.generatedAt).toLocaleString()}</td>
                        <td style={{ padding: '6px 10px' }}>
                          <button onClick={() => downloadReport(r.id)} style={{ ...btnSec, padding: '3px 8px', fontSize: 11.5 }}>{t('minedubStats.downloadBtn')}</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
