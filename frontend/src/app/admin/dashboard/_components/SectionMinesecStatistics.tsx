...
const btnPri = { padding: '7px 14px', borderRadius: 8, border: 'none', background: 'var(--green)', color: '#fff', fontWeight: 700, fontSize: 12.5, cursor: 'pointer' as const }
const btnSec = { padding: '7px 14px', borderRadius: 8, border: '1px solid var(--border)', background: 'var(--surface)', color: 'var(--text)', fontWeight: 600, fontSize: 12.5, cursor: 'pointer' as const }
const btnSmall = { padding: '4px 10px', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--surface)', color: 'var(--text)', fontWeight: 600, fontSize: 11.5, cursor: 'pointer' as const }
const inputStyle = { padding: '6px 10px', borderRadius: 7, border: '1px solid var(--border)', background: 'var(--bg)', color: 'var(--text)', fontSize: 12.5, width: '100%', boxSizing: 'border-box' as const }
const smallInputStyle = { ...inputStyle, padding: '4px 8px', fontSize: 11.5 }
  ...
  return (
    <div className="px-4 py-4 md:px-6 md:py-5" style={{ height: '100%', overflowY: 'auto' }}>
      <div style={{ marginBottom: 14 }}>
        <h2 className="text-[15px] md:text-[17px]" style={{ fontFamily: 'var(--font-spectral),Spectral,serif', fontWeight: 700, color: 'var(--text)' }}>{t('minesecStats.title')}</h2>
        <p className="text-[11px] md:text-[12px]" style={{ color: 'var(--text3)', marginTop: 2 }}>{t('minesecStats.subtitle')}</p>
      </div>

      <div className="gap-[6px] md:gap-[8px] mb-[12px] md:mb-[14px]" style={{ display: 'flex' }}>
        <button onClick={() => setTab('supplement')} className="flex-1 md:flex-none rounded-[8px] text-[11.5px] md:text-[12.5px] py-[6px] px-3 md:px-[16px]" style={{ ...(tab === 'supplement' ? btnPri : btnSec), padding: undefined }}>{t('minesecStats.tabSupplement')}</button>
        <button onClick={() => setTab('generer')} className="flex-1 md:flex-none rounded-[8px] text-[11.5px] md:text-[12.5px] py-[6px] px-3 md:px-[16px]" style={{ ...(tab === 'generer' ? btnPri : btnSec), padding: undefined }}>{t('minesecStats.tabGenerer')}</button>
      </div>
        ...
        <div>
          {supplement?.lastUpdatedAt && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
              ...
              {isStale && (
                <span style={{ fontSize: 10.5, fontWeight: 700, color: '#b45309', background: 'rgba(234,179,8,0.12)', padding: '2px 8px', borderRadius: 10 }}>
                  {t('minesecStats.staleWarning')}
                </span>
              )}
            </div>
          )}
          <p style={{ fontSize: 12, color: 'var(--text3)', marginBottom: 12 }}>{t('minesecStats.persistenceExplainer')}</p>

          {/* Identification */}
          <div className={cardStyleCls} style={cardStyle}>
            <h3 className="text-[13px] md:text-[14px]" style={{ fontWeight: 700, color: 'var(--text)', marginBottom: 4 }}>{t('minesecStats.sectionIdentification')}</h3>
            ...
            <div className="grid grid-cols-2 sm:flex gap-2 md:gap-2.5">
              {(['moins1km', 'entre1et3km', 'entre4et5km', 'plus5km'] as const).map((key) => (
                <div key={key} className="sm:flex-1">
                  <div style={{ fontSize: 10.5, color: 'var(--text3)', marginBottom: 3 }}>{t(`minesecStats.ecolesPrimaires_${key}`)}</div>
          ...
          <div className={cardStyleCls} style={cardStyle}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
              <h3 className="text-[13px] md:text-[14px]" style={{ fontWeight: 700, color: 'var(--text)' }}>{t('minesecStats.sectionBip')}</h3>
              <button onClick={addBip} style={btnSmall}>+ {t('minesecStats.addBip')}</button>
            </div>
            {(form.historiqueBip ?? []).length === 0 && (
              <p style={{ fontSize: 12, color: 'var(--text3)' }}>{t('minesecStats.bipEmpty')}</p>
            )}
            {(form.historiqueBip ?? []).map((bip: any, idx: number) => (
              <div key={idx} className="flex-wrap md:flex-nowrap" style={{ display: 'flex', gap: 8, marginBottom: 6, alignItems: 'center' }}>
                ...
                <input style={{ ...inputStyle, maxWidth: 110 }} placeholder={t('minesecStats.bipAnnee')} value={bip.anneeObtention} onChange={(e) => updateBip(idx, 'anneeObtention', e.target.value)} />
                <button onClick={() => removeBip(idx)} style={{ ...btnSec, padding: '5px 10px', fontSize: 12, color: 'var(--red)', borderColor: 'var(--red)' }}>×</button>
              </div>
            ))}
          </div>
          ...
          {meta && (
            <div className={cardStyleCls} style={cardStyle}>
              <h3 className="text-[13px] md:text-[14px]" style={{ fontWeight: 700, color: 'var(--text)', marginBottom: 3 }}>{t('minesecStats.sectionInfrastructures')}</h3>
              <p style={{ fontSize: 11.5, color: 'var(--text3)', marginBottom: 12 }}>{t('minesecStats.infrastructuresHint')}</p>

              {meta.subsystems.map((s) => (
                <div key={s.code} style={{ marginBottom: 16 }}>
                  {meta.subsystems.length > 1 && (
                    <h4 style={{ fontSize: 12, fontWeight: 700, color: 'var(--text2)', marginBottom: 6 }}>{s.label}</h4>
                  )}
                      ...
                      return (
                        <div key={row.code} style={{ borderTop: '1px solid var(--bg)' }}>
                          <div onClick={() => setExpandedInfraRow(isOpen ? null : rowKey)}
                            ...
                            <span className="gap-[8px] md:gap-[12px]" style={{ fontSize: 11.5, color: 'var(--text3)', display: 'flex' }}>
                              <span>{t('minesecStats.infraTotalLocaux')}: <strong style={{ color: 'var(--text)' }}>{total}</strong></span>
                              <span>{isOpen ? '▲' : '▼'}</span>
                            </span>
                          </div>
                          {isOpen && (
                            <div style={{ padding: '4px 12px 12px', background: 'var(--bg2)' }}>
                              <div style={{ marginBottom: 8 }}>
                                <div style={{ fontSize: 10.5, color: 'var(--text3)', marginBottom: 3 }}>{t('minesecStats.infraPlaces')}</div>
                                <input style={{ ...smallInputStyle, maxWidth: 130 }} type="number" min={0}
                                  value={places ?? ''}
                              ...
                              <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                                {meta.infraBreakdownKeys.filter((b) => b.key !== 'placesDisponibles').map((b) => (
                                  <div key={b.key} style={{ minWidth: 100 }}>
                                    <div style={{ fontSize: 10.5, color: 'var(--text3)', marginBottom: 3 }}>{b.label}</div>
              ...
              <h4 style={{ fontSize: 13, fontWeight: 700, color: 'var(--text)', margin: '16px 0 8px' }}>{t('minesecStats.sectionCommodites')}</h4>
              {meta.commoditesRows.map((row) => (
                <div key={row.code} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '5px 0', borderBottom: '1px solid var(--bg)' }}>
          ...
          {meta && meta.specialitesTechniques.length > 0 && (
            <div className={cardStyleCls} style={cardStyle}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 3 }}>
                <h3 className="text-[13px] md:text-[14px]" style={{ fontWeight: 700, color: 'var(--text)' }}>{t('minesecStats.sectionEstp')}</h3>
                <button onClick={addEstpEntry} style={btnSmall}>+ {t('minesecStats.addEstp')}</button>
              </div>
              <p style={{ fontSize: 11.5, color: 'var(--text3)', marginBottom: 12 }}>{t('minesecStats.estpHint')}</p>

              {(form.effectifsTechniquesDetail ?? []).length === 0 && (
                <p style={{ fontSize: 12, color: 'var(--text3)' }}>{t('minesecStats.estpEmpty')}</p>
              )}
              ...
              {(form.effectifsTechniquesDetail ?? []).map((entry: EstpEntry, idx: number) => {
                ...
                return (
                  <div key={idx} style={{ border: '1px solid var(--border)', borderRadius: 8, padding: 10, marginBottom: 8 }}>
                    <div className="flex-wrap sm:flex-nowrap" style={{ display: 'flex', gap: 8, marginBottom: 6 }}>
                      <div className="flex-1 min-w-[150px] sm:flex-[2]">
                        <div style={{ fontSize: 10.5, color: 'var(--text3)', marginBottom: 3 }}>{t('minesecStats.estpSpecialite')}</div>
                      ...
                      <div className="flex-1 min-w-[90px]">
                        <div style={{ fontSize: 10.5, color: 'var(--text3)', marginBottom: 3 }}>{t('minesecStats.estpAnnee')}</div>
                      ...
                      <button onClick={() => removeEstpEntry(idx)} style={{ ...btnSec, padding: '4px 10px', fontSize: 11.5, color: 'var(--red)', borderColor: 'var(--red)', alignSelf: 'flex-end' }}>×</button>
                    </div>
                    <div className="grid grid-cols-3 sm:flex gap-2">
                      {([
                        ...
                        <div key={key} className="sm:flex-1">
                          <div style={{ fontSize: 10.5, color: 'var(--text3)', marginBottom: 3 }}>{t(`minesecStats.${labelKey}`)}</div>
                    ...
                    {overCapacity && (
                      <p style={{ fontSize: 10.5, color: 'var(--red)', marginTop: 6 }}>{t('minesecStats.estpOverCapacity')}</p>
                    )}
                  </div>
                )
              })}
          ...
          {champsManquants.length > 0 && (
            <div style={{ background: 'rgba(239,68,68,0.08)', border: '1px solid var(--red)', borderRadius: 10, padding: 12, marginBottom: 14 }}>
              <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--red)', marginBottom: 6 }}>{t('minesecStats.champsManquantsTitle')}</p>
              <ul style={{ margin: 0, paddingLeft: 16 }}>
                {champsManquants.map((c) => <li key={c.champ} style={{ fontSize: 12, color: 'var(--text2)' }}>{c.label}</li>)}
              </ul>
            </div>
          )}
      ...
      {tab === 'generer' && (
        <div>
          <div className={cardStyleCls} style={cardStyle}>
            <p className="text-[11.5px] md:text-[12.5px]" style={{ color: 'var(--text2)', marginBottom: 12, lineHeight: 1.5 }}>{t('minesecStats.generateDescription')}</p>
          ...
          {lastSubmission && (
            <div className={cardStyleCls} style={{ ...cardStyle, border: '1.5px solid var(--green)' }}>
              <p className="text-[12.5px] md:text-[13px]" style={{ fontWeight: 700, color: 'var(--green)', marginBottom: 6 }}>{t('minesecStats.generationSuccess')}</p>
                        ...
                        <thead>
                          <tr>
                            {[t('minesecStats.colChamp'), t('minesecStats.colFeuille'), t('minesecStats.colRaison')].map((h) => (
                              <th key={h} style={{ padding: '6px 10px', textAlign: 'left', background: 'var(--bg2)', color: 'var(--text3)', fontWeight: 700 }}>{h}</th>
                            ))}
                          </tr>
                        </thead>
                        <tbody>
                          {champsNonResolus.map((c, i) => (
                            <tr key={i} style={{ borderTop: '1px solid var(--bg)' }}>
                              <td style={{ padding: '5px 10px', color: 'var(--text)' }}>{c.fieldLabel}</td>
                              <td style={{ padding: '5px 10px', color: 'var(--text2)' }}>{c.sheetName}</td>
                              <td style={{ padding: '5px 10px', color: 'var(--text3)' }}>{c.raison}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
          ...
          <div className={cardStyleCls} style={cardStyle}>
            <h3 className="text-[13px] md:text-[14px]" style={{ fontWeight: 700, color: 'var(--text)', marginBottom: 8 }}>{t('minesecStats.historyTitle')}</h3>
            {submissions.length === 0 ? (
              <p style={{ fontSize: 12, color: 'var(--text3)' }}>{t('minesecStats.historyEmpty')}</p>
            ) : (
              <>
              {/* ── Cartes empilées — mobile ── */}
              <div className="md:hidden flex flex-col" style={{ gap: 6 }}>
                {submissions.map((s) => (
                  <div key={s.id} className="rounded-[10px] shadow-[0_1px_2px_rgba(20,20,15,0.05)]" style={{ background: 'var(--surface)', padding: '10px 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
                    <div>
                      <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text)' }}>{new Date(s.generatedAt).toLocaleString()}</div>
                      <div style={{ fontSize: 10.5, color: 'var(--text3)', marginTop: 2 }}>{s.status}</div>
                    </div>
                    {s.filePath && <button onClick={() => downloadSubmission(s.id)} style={{ padding: '5px 10px', borderRadius: 7, fontSize: 11, fontWeight: 700, background: 'var(--bg2)', color: 'var(--text)', border: 'none', cursor: 'pointer', fontFamily: 'inherit', flexShrink: 0 }}>{t('minesecStats.downloadBtn')}</button>}
                  </div>
                ))}
              </div>
              {/* ── Tableau — desktop ── */}
                  ...
                  <tbody>
                    {submissions.map((s) => (
                      <tr key={s.id} style={{ borderTop: '1px solid var(--bg)' }}>
                        <td style={{ padding: '6px 10px', color: 'var(--text2)' }}>{new Date(s.generatedAt).toLocaleString()}</td>
                        <td style={{ padding: '6px 10px', color: 'var(--text2)' }}>{s.status}</td>
                        <td style={{ padding: '6px 10px' }}>
                          {s.filePath && <button onClick={() => downloadSubmission(s.id)} style={{ ...btnSec, padding: '3px 8px', fontSize: 11.5 }}>{t('minesecStats.downloadBtn')}</button>}
                        </td>
                      </tr>
                    ))}
                  </tbody>
