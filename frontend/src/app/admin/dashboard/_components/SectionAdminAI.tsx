  ...
  return (
    <div className="px-4 py-4 md:px-6 md:py-5" style={{ overflowY: 'auto', height: '100%' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10, marginBottom: 16 }}>
        <div>
          <div className="text-[15px] md:text-[17px]" style={sTitle}>{t('ai.title')}</div>
          <div className="text-[11px] md:text-[12px]" style={sSub}>{t('ai.subtitle')}</div>
          {fromCache && cachedAt && (
            <div className="text-[11px] md:text-[12px]" style={{ background: 'var(--amber-light)', border: '1px solid var(--amber)', borderRadius: 6, padding: '3.5px 9px', fontWeight: 600, color: 'var(--amber)', display: 'inline-flex', alignItems: 'center', gap: 5, marginTop: 6 }}>
        ...
        <button
          ...
          onClick={refetch}><RefreshCw size={13} /> {t('ai.btn_refresh')}</button>
      </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5" style={{ gap: 10, marginBottom: 14 }}>
          {(Object.entries(ALERT_STYLE) as [string, typeof ALERT_STYLE[string]][]).map(([key, s]) => {
            ...
            <div key={key}
              ...
              <div className="text-[17px] md:text-[19px] font-bold md:font-black" style={{ color: s.color }}>{summary[key as keyof HealthSummary]}</div>
              <div className="text-[11px] md:text-[11.5px]" style={{ color: 'var(--text3)', fontWeight: 600, marginTop: 2 }}>{alertLabel(key)}</div>
            </div>
            )
          })}
        </div>
      ...
      {atRisk > 0 && !loading && (
        <div className="text-[12px] md:text-[13.5px] px-[12px] py-[8px] md:px-[16px] md:py-[9px]" style={{ background: 'var(--red-light)', border: '1px solid rgba(220,38,38,0.2)', borderRadius: 9, marginBottom: 12, display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      <div style={{ display: 'flex', gap: 7, marginBottom: 12, flexWrap: 'wrap', alignItems: 'center' }}>
        <select value={classFilter} onChange={e => setClassFilter(e.target.value)} className="text-[12px] md:text-[13px] px-[10px] py-[6px] md:px-[10px] md:py-[6px]" style={{ ...filterSt }}>
          <option value="">{t('ai.filter_all_classes')}</option>
          {classes.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
        </select>
        {alertFilter && (
          <button onClick={() => setAlertFilter('')} className="text-[11.5px] md:text-[12.5px] px-[9px] py-[5px] md:px-[10px] md:py-[5px]" style={{ borderRadius: 7, fontWeight: 700, background: 'var(--red-light)', color: 'var(--red)', border: '1px solid rgba(220,38,38,0.3)', cursor: 'pointer', fontFamily: 'inherit', display: 'inline-flex', alignItems: 'center', gap: 4 }}>
            <X size={12} /> {alertLabel(alertFilter)}
          </button>
        )}
        <span className="text-[11.5px] md:text-[12.5px]" style={{ marginLeft: 'auto', color: 'var(--text3)', fontWeight: 600, alignSelf: 'center' }}>
      ...
      <div className="rounded-none md:rounded-[12px] border-0 md:border md:border-[1.5px] md:border-[var(--border)] bg-transparent md:bg-[var(--surface)]" style={{ overflow: 'hidden' }}>
        {loading ? (
          ...
          <div style={{ padding: '30px 16px', textAlign: 'center', color: 'var(--red)', fontWeight: 700, fontSize: 13 }}>{error}</div>
        ) : filtered.length === 0 ? (
          <div style={{ padding: '36px 16px', textAlign: 'center', color: 'var(--text3)' }}>
            <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 10 }}>
              <Bot size={28} className="md:hidden" />
              <Bot size={36} className="hidden md:block" />
            </div>
            <div className="text-[13px] md:text-[14.5px]">{t('ai.empty_filtered')}</div>
          </div>
        ) : (
          <>
          ...
          <div className="md:hidden flex flex-col" style={{ gap: 8 }}>
            {filtered.map(s => {
              ...
              return (
                <div key={s.studentId} className="rounded-[12px] shadow-[0_1px_2px_rgba(20,20,15,0.05),0_1px_6px_rgba(20,20,15,0.06)]" style={{ background: 'var(--surface)', padding: 10 }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8 }}>
                    <div>
                      <div style={{ fontWeight: 700, color: 'var(--text)', fontSize: 13 }}>{s.name}</div>
                      <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 1.5 }}>{s.className}</div>
                    </div>
                    <span style={{ padding: '2.5px 7.5px', borderRadius: 14, fontSize: 10.5, fontWeight: 700, background: al.bg, color: al.color, display: 'inline-flex', alignItems: 'center', gap: 3.5, flexShrink: 0 }}>
                      <al.icon size={10.5} /> {alertLabel(s.alertLevel)}
                    </span>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 7 }}>
                    <div style={{ flex: 1, height: 6, background: 'var(--bg2)', borderRadius: 3, overflow: 'hidden' }}>
                      <div style={{ width: `${s.healthScore}%`, height: '100%', background: barColor, borderRadius: 3, transition: 'width 0.5s' }} />
                    </div>
                    ...
                    <tr key={s.studentId}
                      ...
                      <td style={tdSt}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          <div style={{ width: 90, height: 6, background: 'var(--bg2)', borderRadius: 3, overflow: 'hidden' }}>
                            <div style={{ width: `${s.healthScore}%`, height: '100%', background: barColor, borderRadius: 3, transition: 'width 0.5s' }} />
                          </div>
                      ...
                      <td style={tdSt}>
                        <span style={{ padding: '2.5px 8px', borderRadius: 14, fontSize: 11.5, fontWeight: 700, background: al.bg, color: al.color, display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                          <al.icon size={11} /> {alertLabel(s.alertLevel)}
                        </span>
                      </td>
                    </tr>
                  )
...
const filterSt: React.CSSProperties = { background: 'var(--surface)', border: '1.5px solid var(--border2)', borderRadius: 7, fontWeight: 700, color: 'var(--text2)', outline: 'none', fontFamily: 'inherit' }
