  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 1000, padding: 12, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div onClick={event => event.stopPropagation()} className="p-4 sm:p-7 rounded-2xl w-[980px] max-w-full max-h-[92vh] overflow-y-auto" style={{ background: 'var(--surface)', boxShadow: '0 32px 80px rgba(0,0,0,0.22)' }}>
        ...
        {step === 2 && <div><label style={{ display: 'block', padding: 20, border: '2px dashed var(--border)', borderRadius: 14, textAlign: 'center', cursor: 'pointer', color: 'var(--text2)' }}><FileSpreadsheet size={32} style={{ margin: 'auto' }} /><strong style={{ display: 'block', marginTop: 8, fontSize: 13 }}>{loading ? t('users.import_modal.loading') : t('users.import_modal.upload_prompt')}</strong><small style={{ color: 'var(--text3)', fontSize: 11.5 }}>{t('users.import_modal.upload_hint')}</small><input type="file" accept=".xlsx,.xls" onChange={event => { const file = event.target.files?.[0]; if (file) void uploadFile(file) }} style={{ display: 'none' }} /></label>
          {preview && <><h3 style={sectionTitle}>{t('users.import_modal.mapping_title')}</h3><div style={{ overflowX: 'auto' }}><table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 400 }}><thead><tr style={{ background: 'var(--bg2)' }}><th style={cellHeader}>{t('users.import_modal.detected_column')}</th><th style={cellHeader}>{t('users.import_modal.target_field')}</th></tr></thead><tbody>{preview.headers.map(header => <tr key={header} style={{ borderTop: '1px solid var(--border)' }}><td style={cell}>{header}</td><td style={cell}><select value={mapping[header] ?? ''} onChange={event => setMapping(current => ({ ...current, [header]: event.target.value }))} style={{ width: '100%', padding: 8, border: '1px solid var(--border)', borderRadius: 7, background: 'var(--surface)', color: 'var(--text)', fontSize: 12.5 }}><option value="">{t('users.import_modal.ignore_column')}</option>{preview.targetFields.map(field => <option key={field} value={field}>{t(`users.import_modal.target_fields.${field}`)}</option>)}</select></td></tr>)}</tbody></table></div><h3 style={sectionTitle}>{t('users.import_modal.preview', { shown: preview.sampleRows.length, total: preview.totalRows })}</h3><div style={{ overflowX: 'auto' }}><table style={{ width: '100%', minWidth: 500, borderCollapse: 'collapse', fontSize: 12 }}><thead><tr style={{ background: 'var(--bg2)' }}>{preview.headers.map(header => <th key={header} style={cellHeader}>{header}</th>)}</tr></thead><tbody>{preview.sampleRows.map((row, index) => <tr key={index} style={{ borderTop: '1px solid var(--border)' }}>{preview.headers.map(header => <td key={header} style={cell}>{row[header] || '—'}</td>)}</tr>)}</tbody></table></div><div className="flex flex-col-reverse sm:flex-row gap-2.5 mt-4"><button type="button" onClick={() => setStep(1)} style={secondaryButton}>{t('users.import_modal.btn_back')}</button><button type="button" onClick={() => void validateRows()} disabled={loading} style={primaryButton}>{t('users.import_modal.btn_next')}</button></div></>}</div>}

        {step === 3 && validation && <div><div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', color: 'var(--text2)', fontSize: 12.5, marginBottom: 12 }}><span>{t('users.import_modal.valid_count', { count: validation.validCount })}</span><span>{t('users.import_modal.warning_count', { count: validation.warningCount })}</span><span>{t('users.import_modal.error_count', { count: validation.errorCount })}</span></div><h3 style={sectionTitle}>{t('users.import_modal.correction_title')}</h3><p style={{ color: 'var(--text3)', fontSize: 12.5 }}>{t('users.import_modal.correction_hint')}</p><div className="overflow-x-auto"><ImportValidationGrid headers={preview?.headers ?? []} rows={validation.validatedRows} columnMapping={mapping} onRowsChange={setRows} /></div><div className="flex flex-col-reverse sm:flex-row gap-2.5 mt-4"><button type="button" onClick={() => void validateRows()} disabled={loading} style={secondaryButton}>{t('users.import_modal.revalidate')}</button><button type="button" onClick={() => void confirmImport()} disabled={!canConfirm || loading} style={{ ...primaryButton, opacity: canConfirm ? 1 : 0.55, cursor: canConfirm ? 'pointer' : 'not-allowed' }}>{t('users.import_modal.btn_confirm')}</button></div></div>}

        {step === 4 && summary && <div style={{ textAlign: 'center' }}><CheckCircle2 size={44} color="var(--green)" strokeWidth={1.5} /><h3 style={{ color: 'var(--text)' }}>{t('users.import_modal.summary_title')}</h3><p style={{ color: 'var(--text2)', fontSize: 13 }}>{t('users.import_modal.result_success', { count: summary.success })} / {summary.total}</p>{summaryKeys[targetType!].map(key => <p key={key} style={{ color: 'var(--text2)', margin: 6, fontSize: 12.5 }}>{t(`users.import_modal.summary.${key}`, { count: summary[key] as number })}</p>)}{[...summary.errors, ...summary.warnings].length > 0 && <div style={{ textAlign: 'left', marginTop: 16, padding: 12, borderRadius: 10, background: 'var(--amber-light)', color: 'var(--text2)', fontSize: 12 }}><AlertTriangle size={15} style={{ verticalAlign: 'middle', marginRight: 6 }} />{t('users.import_modal.remaining_issues')}{summary.errors.map(issue => <p key={`error-${issue.ligne}`}>{t('users.import_modal.line', { line: issue.ligne })} — {issue.erreur}</p>)}{summary.warnings.map(issue => <p key={`warning-${issue.ligne}`}>{t('users.import_modal.line', { line: issue.ligne })} — {issue.avertissement}</p>)}</div>}<div className="flex flex-col-reverse sm:flex-row gap-2.5 mt-4"><button type="button" onClick={reset} style={secondaryButton}>{t('users.import_modal.btn_import_another')}</button><button type="button" onClick={onClose} style={primaryButton}>{t('users.import_modal.btn_close')}</button></div></div>}
      </div>
    </div>
  )
}
