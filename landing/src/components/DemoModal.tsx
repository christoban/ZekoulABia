...
export default function DemoModal({ isOpen, onClose, onSuccess, onError, lang }: DemoModalProps) {
  ...
  const submit = async () => {
    ...
    try {
      // Chemin relatif : next.config.ts fait suivre /api/v2/* vers le backend Express, même
      // pattern que l'app principale (frontend/next.config.ts) — pas de CORS à configurer côté
      // backend, un seul domaine vu par le navigateur.
